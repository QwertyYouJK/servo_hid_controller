...
uint8_t g_hid_rx_buf[HID_OUT_LEN];
...
void USBD_CUSTOM_HID_RecvReport_Callback(uint8_t *buf, uint32_t len);
...
static int8_t CUSTOM_HID_OutEvent_FS(uint8_t event_idx, uint8_t state) {
	...
	uint32_t rx_len = USBD_LL_GetRxDataSize(&hUsbDeviceFS,
	CUSTOM_HID_EPOUT_ADDR);

	USBD_CUSTOM_HID_RecvReport_Callback(hhid->Report_buf, rx_len);

	USBD_LL_PrepareReceive(&hUsbDeviceFS, CUSTOM_HID_EPOUT_ADDR,
	                       hhid->Report_buf, HID_OUT_LEN);
